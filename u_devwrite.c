  ...
  char * argv[])
{
  ...
  {
	fprintf (stderr, "u_devdevwrite: Warning: You have %iMB RAM, but you test with only %iMB datasize!\n",
		memsz/(1024*1024), size);
	fprintf (stderr, "u_devdevwrite:          This might yield unrealistically good results,\n");
	fprintf (stderr, "u_devdevwrite:          for reading and seeking%s.\n",
		 (dosync? "": " and writing"));
  }
#endif
  ...
  sleep (1); sync ();
   ...
   fprintf(stderr, "Device write speed test\n");
   fprintf(stderr, "Device name: %s     Device Size: %d\n",filename,dev_size);
   fprintf(stderr, "\n"); 
   fflush(stderr);

   if(dev_size>size*2)
        start_addr = lseek(fd,dev_size/2,SEEK_SET);
   else if(dev_size>size)
        start_addr = lseek(fd,dev_size-size,SEEK_SET);
   else
   ...
   while(item<MAX_ITEM)
   {
       ...
       while(words<size)
       {
            ...
            if(read(fd, (char *) buf, Chunk)<Chunk)
            {
                fprintf(stderr,"read error");    
                break;
            }

            if (lseek(fd, (off_t) -Chunk, SEEK_CUR) == -1)            
            {
            ...
            temp = 0;
            while(temp+blocksize<=Chunk)
            {
                if(write(fd,(char *)buf+temp,blocksize)<blocksize)
                {
